...
class OWdyndiffraction(widget.OWWidget):
    ...
    def calculate_external_dyndiffraction(ENERGY=10000.0,MIN_ANGLE=0.995,\
        ...
        cryst_0 = reflectivity.crystal(names[CRYSTAL_NAME_0])
        substrate=reflectivity.Substrate(cryst_0)
        layers.append(substrate)
        Miller_0=(eval(MILLER_0))
        v_perp_0=sp.Matrix(eval(V_PERP_0))
        if ORIENTATION_INPUT==0:
            substrate.calc_orientation_from_angle(PSI_0, v_perp_0)
        elif ORIENTATION_INPUT==1:
            v_par_0=sp.Matrix(eval(V_PAR_0))
            substrate.calc_orientation(v_par_0, v_perp_0)
        else:
            raise Exception("ORIENTATION_INPUT is not valid")
        ...
        if N_LAYERS>0:
            ...
            if ORIENTATION_INPUT==0:
                ...
                v_par_1=sp.Matrix(eval(V_PAR_1))
                layer_1.calc_orientation(v_par_1, v_perp_1)
            else:
                raise Exception("ORIENTATION_INPUT is not valid")

                if N_LAYERS>1:
                    cryst_2 = reflectivity.crystal(names[CRYSTAL_NAME_2])
                    layer_2=reflectivity.Epitaxial_Layer(cryst_2, THICKNESS_2)
                    layers.append(layer_2)
                    v_perp_2=sp.Matrix(eval(V_PERP_2))
                    if ORIENTATION_INPUT==0:
                        layer_2.calc_orientation_from_angle(PSI_2, v_perp_2)
                    elif ORIENTATION_INPUT==1:
                        v_par_2=sp.Matrix(eval(V_PAR_2))
                        layer_2.calc_orientation(v_par_2, v_perp_2)
                    else:
                        raise Exception("ORIENTATION_INPUT is not valid")
        crystal_sample=reflectivity.Sample(*layers)
        crystal_sample.set_Miller(Miller_0)
        crystal_sample.calc_g0_gH(ENERGY)
        thBragg=float(substrate.calc_Bragg_angle(ENERGY).subs(substrate.structure.lattice_par_val).evalf())
